...
public class AccessManagerService {
	...
	public List<RolesDto> getRoleList() {
		...
		List<RolesDto> rolesDto = roles.stream().map(this::copyRoleEntityToDto).toList();
		return rolesDto;
	}

	private RolesDto copyRoleEntityToDto(RoleMaster role) {
		RolesDto roleDto = new RolesDto();
		List<RolePermissionsDto> rolePermissionsDtoList = new ArrayList<>();
		Utils.copyProperties(role, roleDto);

		if (role.getId() == Constants.SUPER_ADMIN.longValue()) {
