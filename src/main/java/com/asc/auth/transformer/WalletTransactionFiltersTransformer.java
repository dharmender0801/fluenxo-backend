...
public class WalletTransactionFiltersTransformer {
	public static Specification<?> buildCriteria(List<FiltersDto> filters) {
		...
		if (Boolean.TRUE.equals(Utils.checkCollectionIsNotEmpty(filters))) {
			filters.forEach(filter -> {
				if (FilterKeys.WALLET_ID.equals(filter.getKey())) {
					filterByWalletId(qb, filter.getKey(), filter.getValue());
	...
	private static void filterByWalletId(QuerySpecificationBuilder qb, FilterKeys key, String value) {
		if (FilterKeys.WALLET_ID.equals(key) && Objects.nonNull(value)) {
			qb.with("wallet", Arrays.asList("id"), SearchOperation.JOIN_MULTI_LONG_LIST_IN, value);
		}
	}
